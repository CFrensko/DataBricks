...
def gb_print_performance_metrics(predictions):
  # Evaluate model
 # evaluator = BinaryClassificationEvaluator(rawPredictionCol="rawPrediction")
 # auc = evaluator.evaluate(predictions, {evaluator.metricName: "areaUnderROC"})
 # aupr = evaluator.evaluate(predictions, {evaluator.metricName: "areaUnderPR"})
...
gb_print_performance_metrics (gbPredictions)
...
#evaluator = classificationEvaluator(metricName="rmse", labelCol=gb.getLabelCol(), predictionCol=gb.getPredictionCol())

from pyspark.ml.classification import GBTClassifier

# Create CrossValidator
gbCv = CrossValidator(estimator=gb, evaluator=evaluator, estimatorParamMaps=gbParamGrid)
...
print(gbCvModel.bestModel._java_obj.getNumTrees())
...
gb_print_performance_metrics(gbCvPredictions)
...
attrs = sorted(
    (attr["idx"], attr["name"]) for attr in (chain(*lrCvPredictions
                           .schema[str(lrCvModel.bestModel.summary.featuresCol)]
...
rfFeatureImportance = rfModel.transform(testData)
display(rfFeatureImportance)
...
# Use test set to measure model accuracy on new data
rfCvPredictions = rfCvModel.transform(testData)
display(rfCvPredictions)

# COMMAND ----------

# Print FeatureImportance
rfCvFeatureImportance = pd.DataFrame([(name, rfCvModel.bestModel.featureImportances[idx]) for idx, name in attrs],columns=['feature_name','feature_importance'])
